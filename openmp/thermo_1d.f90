...
! 1. Temperature from potential temperature
	...
	call cpu_time(time1)
  	...
  	do i=istart,iend
	    ...
	    if (BCC(i,j).gt.0) then
		 ...
		 !print*, HHd_in,SSd_in,HHd_out,SSd_out,HHm_in,HHm_out,SSm_in,SSm_out 
		...
		!print*,ENTHALPY_DA (i,j), ENTHALPYDEL_D (i,j), ENTHALPYDEL_M (i,j), ENTROPY_DA (i,j)
  	     end do
  ...
  call write_netcdf_output(trim(outdir)//trim(fname2(L1:LL)),nx,ny,nz,SIRR_D/3600,&
 				...
 				ENTROPYDEL_M/3600)
       ! print*,ENTHALPYDEL_D(200,180)/3600,ENTROPY_DA(200,180)/3600
