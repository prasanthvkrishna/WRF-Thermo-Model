...
real(dbl) ctrl_density_air
real(dbl) ctrl_eps_exit_air
real(dbl) ctrl_density2_air
...
real(dbl) air_g_si, a_si, t_si, p_si
...
real(dbl) air_density_si, a_si, t_si, p_si
real(dbl) d, d2, eps
integer maxit
...
real(dbl) densityiteration_newton, a, t, p, d, eps
...
real(dbl) densityiteration_brent, a_si, t, p, d1, d2, eps
real(dbl) a, b, c, d, s
...
real(dbl) densityiteration_secant, a, t, p, d, d2, eps
...
real(dbl) air_a_t_p_derivative_si, a_si, t_si, d_si
real(dbl) g
real(dbl) n
real(dbl) f, f_t, f_d, f_tt, f_td, f_dd, f_at, f_ad, f_aa
integer drv_a, drv_t, drv_p
...
real(dbl) get_it_ctrl_airdensity
character*13 key
...
real(dbl) aux_density_ideal_si, a_si, t_si, p_si
real(dbl) ma, r
