...
real(dbl) cnv_pressure, p_in
real(dbl) p_si, lat
real(dbl), optional :: lat0
...
character*(*) unit_out, unit_in
real(dbl) cnv_temperature, t_in, t_c
real(dbl) t_si
...
character*(*) unit_out, unit_in
real(dbl) cnv_salinity, s_in
real(dbl) t, p
real(dbl), optional :: lat0, lon0, t0_si, p0_si
real(dbl) :: t_si, p_si, ploc_si, lon, lat
real(dbl) ups, ucl, c35, t68_c, p_dbar, psal, s_si, c_ratio
...
Select Case(unit_in)
  ...
  Case ("KG/KG(ABS)")
     ...
     s_si = psal_from_asal(s_in, lon, lat, ploc_si) * ups
  Case ("G/KG(ABS)")
     if(.not. (present(lat0) .and. present(lon0) .and. present(p0_si))) then
        write(*,*) 'Position and pressure are needed to convert from or to Absolute Salinity'
        write(*,*) 'in order to determine the Absolute Salinity anomaly'
     ...
     s_si = psal_from_asal(0.001d0 * s_in, lon, lat, ploc_si) * ups
...
Select Case(unit_out)
  ...
  Case ("KG/KG(ABS)")
     ...
     psal = s_si / ups
     cnv_salinity = asal_from_psal(psal, lon, lat, ploc_si)
  Case ("G/KG(ABS)")
     if(.not. (present(lat0) .and. present(lon0) .and. present(p0_si))) then
        write(*,*) 'Position and pressure are needed to convert from or to Absolute Salinity'
        write(*,*) 'in order to determine the Absolute Salinity anomaly'
     ...
     psal = s_si / ups
     cnv_salinity = 1d3 * asal_from_psal(psal, lon, lat, ploc_si)
...
implicit none
real(dbl) t68_k_from_t48_k, t48_k
integer i, k
real(dbl) a48a4, b48, c48c4, a48a68, b48b68, a68, b68
real(dbl) t48, t68, t68alt, tstri
real(dbl) hg1, hg2, hg3, fkt
real(dbl) a(21)
...
implicit none 
real(dbl) t_si_from_t48_k, t48_k
...
implicit none
real(dbl) t_si_from_t68_k, t68_k
integer j
real(dbl) deltat, tn
real(dbl) a(0:12), b(1:8), c(0:5)
...
implicit none
real(dbl) p_si_from_psea_pa, psea_pa
...
implicit none 
real(dbl) psea_pa_from_p_si, p_si
...
implicit none
real(dbl) psea_pa_from_psea_bar, psea_bar
...
implicit none
real(dbl) psea_bar_from_psea_pa, psea_pa
...
real(dbl) sal78_from_cnd, cnd, t, p
real(dbl) dt, res, rt
...
integer n
real(dbl) cnd_from_sal78, cnd, sal78, t, p
real(dbl) dt, rt, si, dels, rtt, at, bt, cp, res
...
integer n, iter
real(dbl) a, xt
real(dbl) dt, res, rt, si, dels, rtt, at, bt, cp
...
real(dbl) p, sw_dpth
real(dbl) deg2rad, c1, c2, c3, c4, gam_dash, x
real(dbl), optional :: lat
...
real(dbl) depth, sw_pres
real(dbl) deg2rad, c1, x
real(dbl), optional :: lat
