...
real(dbl), private :: ctrl_density_liquid, ctrl_density_vapour, ctrl_eps_exit_liquid,  &
...
implicit none
integer maxit
real(dbl) liq_density_si, t_si, p_si
real(dbl) d, d2, eps
...
implicit none
integer maxit
real(dbl) vap_density_si, t_si, p_si
real(dbl) d, d2, eps
...
implicit none
integer it, maxit
real(dbl) densityiteration_newton, t, p, d, eps
real(dbl) dd, f_d, f_dd
...
implicit none
integer it, maxit
real(dbl) densityiteration_brent, t, p, d1, d2, eps
real(dbl) a, b, c, d, s
real(dbl) fa, fb, fc, fs
logical mflag
...
implicit none
integer it, maxit
real(dbl) densityiteration_secant, t, p, d, d2, eps
real(dbl) dd, d1, p1, p2
...
implicit none
integer drv_t, drv_p
real(dbl) flu_t_p_derivative_si, t_si, d_si

real(dbl) g

real(dbl) f, f_t, f_d
real(dbl) f_tt, f_td, f_dd
...
implicit none
real(dbl) aux_density_eos80_si, t_si, p_si
real(dbl) aw, bw, kw, rw
real(dbl) t, p
...
implicit none
real(dbl) aux_density_ideal_si, t_si, p_si, r
...
real(dbl) p1, p2
real(dbl) dc, pc, tc
real(dbl) a0, a1, a2, a3
real(dbl) a10, a20, a11, a21, a12, a22, a03, a13, a23
real(dbl) d_liq, d_vap, t_si, p_si
...
implicit none
real(dbl) aux_liq_density_critical_si, t_si, p_si
real(dbl) d_liq, d_vap
...
implicit none
real(dbl) aux_vap_density_critical_si, t_si, p_si
real(dbl) d_liq, d_vap
...
real(dbl) aux_liq_density_f03_si
real(dbl) t_si, p_si
real(dbl) d
...
integer drv_t, drv_p
real(dbl) aux_liq_g_f03_si
real(dbl) t_si, p_si
real(dbl) t0, tu, p0, pu
real(dbl) y, z, g
...
real(dbl) aux_liq_density_if97_si
real(dbl) t_si, p_si
real(dbl) d
...
real(dbl) aux_vap_density_if97_si
real(dbl) t_si, p_si
real(dbl) d
...
integer drv_t, drv_p
real(dbl) aux_liq_g_if97_si
real(dbl) t_si, p_si
real(dbl) g, g_t, g_tt
real(dbl) g_p, g_tp, g_pp
real(dbl) psi, tau, rt
real(dbl) tu, pu, r
...
real(dbl) g, g_t, g_p
real(dbl) tt, t0, tu, pt, p0, pu, yt, zt
