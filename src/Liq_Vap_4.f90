...
real(dbl), private :: ctrl_init_d_liq, ctrl_init_d_vap, ctrl_init_t, ctrl_eps_exit_p_vap
real(dbl), private :: equi_liq_vap_t, equi_liq_vap_p_liq, &
        equi_liq_vap_p_vap, equi_liq_vap_g_liq, equi_liq_vap_g_vap, &
        equi_liq_vap_d_liq, equi_liq_vap_d_vap
real(dbl), private :: d_liq_si, d_vap_si, g_liq_si, g_vap_si, p_liq_si, p_vap_si
real(dbl), private :: t_si
...
real(dbl) liq_vap_vapourpressure_si
real(dbl) t_si
...
real(dbl) liq_vap_boilingtemperature_si
real(dbl) p_si
...
real(dbl) liq_vap_pressure_vap_si
...
real(dbl) liq_vap_pressure_liq_si
...
real(dbl) liq_vap_temperature_si
...
real(dbl) liq_vap_density_liq_si
...
real(dbl) liq_vap_density_vap_si
...
real(dbl) liq_vap_entropy_liq_si
...
real(dbl) liq_vap_entropy_vap_si
...
real(dbl) liq_vap_enthalpy_liq_si
...
real(dbl) liq_vap_enthalpy_vap_si
...
real(dbl) liq_vap_volume_evap_si
...
real(dbl) liq_vap_entropy_evap_si
real(dbl) sl, sv
...
real(dbl) liq_vap_enthalpy_evap_si
real(dbl) hl, hv
...
integer maxit
real(dbl) set_liq_vap_eq_at_p
real(dbl) t_si, p_si
real(dbl) pl, pv
real(dbl) gl, gv
real(dbl) dl, dv
real(dbl) eps
real(dbl) dc, pc, tc, gc, dl_triple, dv_triple, t_triple
...
integer maxit
real(dbl) set_liq_vap_eq_at_t
real(dbl) t_si
real(dbl) pl, pv
real(dbl) gl, gv
real(dbl) dl, dv
real(dbl) eps
real(dbl) dc, pc, tc, gc, dl_triple, dv_triple
...
implicit none
real(dbl) liq_vap_iteration_at_p

integer it, maxit
real(dbl) eps, t_si, d_liq_si, d_vap_si,  &
...
real(dbl) fv, fv_d, fv_dd
real(dbl) fl_t, fl_td
real(dbl) fv_t, fv_td
real(dbl) a(3, 3), b(3), x(3)
real(dbl) ddl, ddv, dt, pv_old
real(dbl) dc, pc, tc, gc
...
implicit none
real(dbl) liq_vap_iteration_at_t

integer it, maxit
real(dbl) t_si, eps, d_liq_si, d_vap_si, g_vap_si, g_liq_si, &
       p_liq_si, p_vap_si
real(dbl) dl, dv
real(dbl) pl, pv
real(dbl) gl, gv
...
real(dbl) aux_liq_density_critical, t_si
real(dbl) tc, dc
real(dbl) a1, a2, a3
real(dbl) r, s, t
real(dbl) x1, x2, x3
...
real(dbl) aux_vap_density_critical, t_si
real(dbl) tc, dc
real(dbl) a1, a2, a3
real(dbl) r, s, t
real(dbl) x1, x2, x3
...
real(dbl) aux_liq_density_triplepoint, t_si
real(dbl) tt, dt
real(dbl) a1, a2, a3, a4, a5
real(dbl) tr
real(dbl) d
...
implicit none
real(dbl) aux_vap_density_triplepoint, t_si
real(dbl) tt, dt
real(dbl) a1, a2, a3
real(dbl) tr
real(dbl) d
...
implicit none
real(dbl) aux_boil_temperature, p_si
real(dbl) tt, pt
real(dbl) a1, a2
real(dbl) p, q
real(dbl) tr
...
real(dbl) dl_triple, dv_triple, t_triple
...
implicit none
integer it, icheck
real(dbl) pl, pv, rhol, rhov
real(dbl) t, p, zt(3), zp, rho
real(dbl) g, gt, gp, gtt, gpp, gtp, q, gt0, h0, rho0
real(dbl) temp
